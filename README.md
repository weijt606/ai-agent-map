...
| #2&nbsp;(↓) | [mattpocock/skills](https://github.com/mattpocock/skills) | 243.9k | +5,954 | Watchlist (Skills Wave) | **The ten-window #1 streak ends.** ~8,336/week is still more than anything below #4 combined, but the rate is off 24% and it was plainly out-gained, not edged out — past **243k** |
...
| #7&nbsp;(↑) | [TradingAgents](https://github.com/TauricResearch/TradingAgents) | 102.2k | +1,455 | Out of scope (finance-research vertical) | Up one on a rate 45% hotter — a fifth window on this table purely on gain, still not an in-scope agent surface |
...
| #9&nbsp;(↓) | [anthropics/skills](https://github.com/anthropics/skills) | 173.0k | +1,173 | Watchlist (Skills Wave canonical) | Flat a second window (+2%) — Anthropic's reference `.claude/skills` repo reached **173k** |
| #10&nbsp;(new) | [OpenHuman](https://github.com/tinyhumansai/openhuman) | 39.3k | +1,111 | In scope · [profile](agents/openhuman.md) | Takes the seat it missed by 259 stars last window, on a rate **70% hotter** — two strong windows running since being profiled |
...
- **[mattpocock/skills](https://github.com/mattpocock/skills)'s ten-window run at #1 is over** (243.9k, +5,954, ~8,336/week against 11,006). It was out-gained by a clear margin rather than edged out, and its own rate fell 24% — both halves of that matter, because the concentration story this map has been telling depends on the second one continuing.
...
- **The `.claude/skills` wave went from 4 of the top ten to 5 — and this time it actually broadened.** For three windows the wave held four seats with general-purpose collections and no rotation. The fifth seat is a **domain** collection (science), with a second domain collection at #11. Read that as the wave finding verticals, which is a different phenomenon from the concentration inside `mattpocock/skills` the map flagged last window. Both are now true at once.
- **The board re-accelerated broadly**, reversing last window's cool-down: **27 of 47** tracked repos posted a higher weekly rate. Biggest movers up outside the top ten: [agentmemory](https://github.com/rohitg00/agentmemory) +81% (27.9k), [OpenHands](agents/openhands.md) +44% (85.9k), [OpenClaw](agents/openclaw.md) +40%, [CrewAI](agents/crewai.md) +39%. Down hardest: [Flowise](agents/flowise.md) −74%, [CodeWhale](agents/codewhale.md) −60%, [Grok Build](agents/grok-build.md) −49%, [Kimi Code](agents/kimi-code.md) −49%, [QM](agents/qm.md) −47% for a second straight window.
...
- The `.claude/skills` wave **holds five of the top ten**, up from four for the first time since June — and the new seat is [K-Dense-AI/scientific-agent-skills](https://github.com/K-Dense-AI/scientific-agent-skills), a science collection, with [academic-research-skills](https://github.com/Imbad0202/academic-research-skills) just behind at #11. The concentration the map flagged last window is still real — [mattpocock/skills](https://github.com/mattpocock/skills) still out-gains the other general collections combined — but the wave is no longer only concentrating; it is also finding domains. Policy unchanged: curated collections are tracked as Skills Wave entries, the framework end is covered through [Superpowers](agents/superpowers.md).
- Just off the table: [academic-research-skills](https://github.com/Imbad0202/academic-research-skills) 44.8k (+975, rate up 67%), [CodeGraph](agents/codegraph.md) 69.1k (+814, rate up 9%, down from #9) and [OpenClaw](agents/openclaw.md) (+792). [n8n](agents/n8n.md) lost its seat again, to #17 on a rate 29% cooler (203.1k, +527) — it has now held a seat in exactly one of the last four windows.
