...
| --- | --- | --- | --- | --- | --- |
| #1（↑） | [colbymchenry/codegraph](https://github.com/colbymchenry/codegraph) | 37.5k | +21,100 | 已收录 · [profile](agents/codegraph.md) | 本窗口的黑马——一口气翻倍多（16.4k → 37.5k）。这个预索引代码知识图谱现在为 [Claude Code](agents/claude-code.md)、[Codex](agents/codex.md)、Gemini、[Cursor](agents/cursor.md)、OpenCode、AntiGravity、Kiro、[Hermes Agent](agents/hermes-agent.md) 建索引；agent 上下文基础设施正在成为独立的需求中心 |
| #2 | [mattpocock/skills](https://github.com/mattpocock/skills) | 114.7k | +13,800 | 候补（Skills 浪潮） | 仍在加速——Matt Pocock 的 `.claude/skills` 个人技能集又涨了约 1.4 万，越过 114k |
| #3 | [Hermes Agent](https://github.com/NousResearch/hermes-agent) | 176.6k | +13,500 | 已收录 · [profile](agents/hermes-agent.md) | 连续第六个窗口增长——已收录项目里绝对总数仍居第一（已突破 176k） |
| #4 | [Superpowers](https://github.com/obra/superpowers) | 215.5k | +12,700 | 已收录 · [profile](agents/superpowers.md) | agentic skills 框架突破 215k——浪潮的框架锚点持续复利增长 |
| #5 | [Imbad0202/academic-research-skills](https://github.com/Imbad0202/academic-research-skills) | 26.0k | +7,000 | 候补（Skills 浪潮） | 本窗口增速最快的 curated skills 集——面向 Claude Code 的学术研究管线，已越过 25k |
| #6 | [anthropics/skills](https://github.com/anthropics/skills) | 145.5k | +6,200 | 候补（Skills 浪潮源头） | Anthropic 自家的 `.claude/skills` 参考仓库——上游源头；增长稳定但已不是浪潮里增量最大的那个 |
| #7 | [Pi](https://github.com/earendil-works/pi) | 58.8k | +5,900 | 已收录 · [profile](agents/pi.md) | 重新提速——Earendil 接手的 harness 在深度定制 playbook 落地后又涨了约 6k |
| #8 | [openhuman](https://github.com/tinyhumansai/openhuman) | 30.4k | +4,700 | 已收录 · [profile](agents/openhuman.md) | 连续第三个强劲窗口——生活集成桌面 agent 越过 30k |
| #9 | [agentmemory](https://github.com/rohitg00/agentmemory) | 20.6k | +4,200 | 候补（memory 层） | 提速并越过 20k——agent 记忆基础设施，与 [MemGPT](agents/memgpt.md) 并列跟踪 |
| #10 | [TradingAgents](https://github.com/TauricResearch/TradingAgents) | 82.1k | +3,500 | 不收录 | 垂直的金融研究多 agent 系统——总数高、增长稳，但作为领域垂直不收录 |

- 热度适合拿来发现新项目，不适合直接当选型顺序。
- 本次没有新增 in-scope profile——这次是热度刷新 + 对齐修正。黑马是一个已收录条目：[codegraph](agents/codegraph.md) 翻倍多，进一步印证「agent 上下文基础设施」值得作为独立路线持续观察。
- **更名跟踪：** 本仓库以 DeepSeek-TUI 收录的项目已改名为 [CodeWhale](agents/codewhale.md)（`Hmbown/CodeWhale`，现 36.6k，+3.1k），并从只支持 DeepSeek 扩展到 **DeepSeek + MiMo**。旧的 `DeepSeek-TUI` 链接会重定向；本仓库的 profile 和各表已统一改用新名。
- **新进雷达：** [jcode](https://github.com/1jehuang/jcode)（1jehuang，「Coding Agent Harness」，Rust，6.8k）作为全新的 harness 框架登上 GitHub Trending——加入候补，与 [SWE-agent](agents/swe-agent.md) / [mini-swe-agent](agents/mini-swe-agent.md) 和 [OpenHarness](agents/openharness.md) 并列。
- `.claude/skills` 浪潮仍在继续，但已不再是唯一主线：前 10 中有 4 个是 skills 集合或框架（`mattpocock/skills`、`Superpowers`、`academic-research-skills`、`anthropics/skills`），比上周的 6 个回落，因为代码上下文和 harness 基础设施抢走了更多热度。策略不变：curated 集合作为 Skills 浪潮条目跟踪，框架那一端通过 [Superpowers](agents/superpowers.md) 覆盖。
- [OpenClaw](agents/openclaw.md) 仍是绝对总数第一，376.2k star；它已有 profile，但因为这种体量的项目周环比增量噪声太大，不进按增量排名的表。
- 本周仍在涨但没进前 10（按增量）：[Codex CLI](https://github.com/openai/codex) 87.9k（+3.2k）、[Ruflo](agents/ruflo.md) 57.4k（+3.2k）、[CodeWhale](agents/codewhale.md) 36.6k（+3.1k）、[addyosmani/agent-skills](https://github.com/addyosmani/agent-skills) 47.7k（+2.8k）、[financial-services](https://github.com/anthropics/financial-services) 29.4k（+2.7k）、[CLI-Anything](agents/cli-anything.md) 41.8k（+2.3k）、[scientific-agent-skills](https://github.com/K-Dense-AI/scientific-agent-skills) 26.9k（+1.7k）、[12-factor-agents](https://github.com/humanlayer/12-factor-agents) 22.9k（+1.2k）。
...
| 路线 | 代表项目 | 常见使用者 |
| --- | --- | --- |
| 直接执行型 | [Claude Code](agents/claude-code.md), [Aider](agents/aider.md), [Codex](agents/codex.md), [CodeWhale](agents/codewhale.md), [Devin](agents/devin.md), [Jules](agents/jules.md) | 想把明确 coding 任务交给 agent 的人 |
