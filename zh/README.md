...
- 这周快照里"不收录"项目特别多：TradingAgents、Pixelle-Video、OpenScreen 都是大涨幅，但都是垂直领域工具（金融 / 视频 / 演示），不是通用 agent 基础设施。
- 已收录项目里最大的新事件是 [Pi](agents/pi.md)——一个小而有主张的 coding agent harness，4 月 8 日被 Earendil 收购后整体走势改变。本周新增 profile。
- Mercury Agent 和 AI Edge Gallery 仍在目录里，但本周增量分别回落到 ~+100 / ~+300，掉出了前 8。
