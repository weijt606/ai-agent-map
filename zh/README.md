...
| --- | --- | --- | --- | --- | --- |
| #1（新） | [TradingAgents](https://github.com/TauricResearch/TradingAgents) | 71.1k | +8,800 | 不收录 | Tauric Research 的多 agent 金融研究框架——本周增量最大，但是垂直领域（金融交易）且作者明确标注"仅供研究"，不是通用 agent 基础设施 |
| #2 | [Hermes Agent](https://github.com/NousResearch/hermes-agent) | 137k | +8,400 | 已收录 · [profile](agents/hermes-agent.md) | 自托管多 agent 环境——连续第三周持续涨，仍是已收录项目里增长最大的 |
| #3（新） | [Pi](https://github.com/earendil-works/pi) | 46k | +5,000 | 已收录 · [profile](agents/pi.md)（本次新增） | 极简的终端 coding agent harness；4 月 8 日被 Earendil（Lefos）收购后获得真实资金，星标增长重新加速 |
| #4（新） | [Pixelle-Video](https://github.com/AIDC-AI/Pixelle-Video) | 13.3k | +4,100 | 不收录 | 端到端视频生成流水线——流程是 agentic，但本质是内容生产工具，不是通用 agent |
| #5 | [Codex CLI](https://github.com/openai/codex) | 80.7k | +1,200 | 已收录 · [profile](agents/codex.md) | OpenAI 终端原生 coding agent——上周 +3.9k 的飙升已降温，但绝对基数已超过 80k |
| #6 | [OpenScreen](https://github.com/siddharthvaddem/openscreen) | 35.1k | +900 | 不收录 | 仍然热门，但本质上是录屏 / 演示工具，不是 agent |
| #7（新） | [ml-intern](https://github.com/huggingface/ml-intern) | 8.9k | +800 | 已收录 · [profile](agents/ml-intern.md)（本次新增） | Hugging Face 的自主 ML 工程师 agent——补上"垂直领域自主 agent"在 map 上的空缺 |
| #8 | [oh-my-claudecode](https://github.com/Yeachan-Heo/oh-my-claudecode) | 32.9k | +700 | 已收录 · [profile](agents/oh-my-claudecode.md) | 围绕 Claude Code 的工作流和 orchestration layer |
| #9 | [GenericAgent](https://github.com/lsdefine/GenericAgent) | 9.5k | +800 | 已收录 · [profile](agents/generic-agent.md)（本次新增） | 复旦团队的自演进 agent——本周从候补升级，覆盖 map 此前缺失的"小种子自演进"细分 |

- 热度适合拿来发现新项目，不适合直接当选型顺序。
- TradingAgents、Pixelle-Video、OpenScreen 这次仍然不收录是有原因的：TradingAgents 是真正基于 LangGraph 的多 agent 框架，但角色都是金融硬编码、作者明确标注"仅供研究"——想要这个模式的读者可以基于 [LangGraph](agents/langgraph.md) 自己搭。Pixelle-Video 是视频生成内容工具（不是可被选型的 agent），OpenScreen 是录屏 / 演示工具，与 agent 选型不直接相关。
- 本次新增三个 in-scope profile：[Pi](agents/pi.md)、[ml-intern](agents/ml-intern.md)、[GenericAgent](agents/generic-agent.md)。后两者前段时间在候补名单，本周观察清楚后正式纳入。
- Mercury Agent 和 AI Edge Gallery 仍在目录里，但本周增量分别回落到 ~+100 / ~+300，掉出了前 9。
...
| 路线 | 代表项目 | 常见使用者 |
| --- | --- | --- |
| 直接执行型 | [Claude Code](agents/claude-code.md), [Aider](agents/aider.md), [Codex](agents/codex.md), [Pi](agents/pi.md), [Devin](agents/devin.md), [Jules](agents/jules.md), [OpenHands](agents/openhands.md) | 想把明确 coding 任务交给 agent 的人 |
