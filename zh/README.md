...
| --- | --- | --- | --- | --- | --- |
| #1（↑） | [addyosmani/agent-skills](https://github.com/addyosmani/agent-skills) | 62.2k | +8,800 | 候补（Skills 浪潮） | 夺回增量榜首——Addy Osmani 的 curated agent-skills 集加速越过 62k |
| #2 | [mattpocock/skills](https://github.com/mattpocock/skills) | 133.3k | +8,200 | 候补（Skills 浪潮） | 仍在复利增长——Matt Pocock 的 `.claude/skills` 个人技能集越过 133k |
| #3（↑） | [Superpowers](https://github.com/obra/superpowers) | 230.9k | +6,600 | 已收录 · [profile](agents/superpowers.md) | agentic skills 框架突破 230k——浪潮的框架锚点持续复利增长 |
| #4（新） | [MiMoCode](https://github.com/XiaomiMiMo/MiMo-Code) | 9.6k | +6,500 | 已收录 · [profile](agents/mimocode.md) | 黑马——小米第一方 MiMo CLI 在被收录后的第一个完整窗口里几乎翻三倍（3.1k → 9.6k），直接冲进增量前 10 |
| #5（↓） | [Hermes Agent](https://github.com/NousResearch/hermes-agent) | 196.1k | +5,600 | 已收录 · [profile](agents/hermes-agent.md) | 从增量榜首降温但仍在涨——已收录项目里绝对总数第一，越过 196k |
| #6（↓） | [colbymchenry/codegraph](https://github.com/colbymchenry/codegraph) | 51.0k | +3,800 | 已收录 · [profile](agents/codegraph.md) | 比黑马窗口慢了但仍在爬——预索引代码知识图谱越过 51k |
| #7 | [anthropics/skills](https://github.com/anthropics/skills) | 152.1k | +2,800 | 候补（Skills 浪潮源头） | Anthropic 自家的 `.claude/skills` 参考仓库——上游源头；稳定增长越过 152k |
| #8（↓） | [Imbad0202/academic-research-skills](https://github.com/Imbad0202/academic-research-skills) | 32.4k | +2,300 | 候补（Skills 浪潮） | 仍是增速很快的 curated skills 集——面向 Claude Code 的学术研究管线越过 32k |
| #9 | [Pi](https://github.com/earendil-works/pi) | 63.6k | +1,900 | 已收录 · [profile](agents/pi.md) | 稳步上行——Earendil 接手的 harness 越过 63k |
| #10（↓） | [TradingAgents](https://github.com/TauricResearch/TradingAgents) | 86.9k | +1,800 | 不收录 | 垂直的金融研究多 agent 系统——总数高、增长稳，但作为领域垂直不收录 |

- 热度适合拿来发现新项目，不适合直接当选型顺序。
- 本窗口头条是 [MiMoCode](agents/mimocode.md)：上周才被收录的小米第一方 MiMo CLI 几乎从 3.1k 翻三倍到 9.6k，在第一个完整窗口就以 #4 冲进增量前 10。
- `.claude/skills` 浪潮重新夺回增量榜首：[addyosmani/agent-skills](https://github.com/addyosmani/agent-skills) 从 #5 跃到 #1 增量位，前 10 中再次有 5 个是 skills 集合或框架（`addyosmani/agent-skills`、`mattpocock/skills`、`Superpowers`、`anthropics/skills`、`academic-research-skills`）。策略不变：curated 集合作为 Skills 浪潮条目跟踪，框架那一端通过 [Superpowers](agents/superpowers.md) 覆盖。
- [Hermes Agent](agents/hermes-agent.md) 从增量榜首降温（#1 → #5）但仍在涨，越过 196k；[Superpowers](agents/superpowers.md) 越过 230k。
- **新收录扫描** 本周发现两个新候补加入 watchlist——[rmux](https://github.com/Helvesec/rmux)（Helvesec，Rust，1.8k，用带类型的 SDK 驱动任意 CLI 的通用多路复用器，与 [CLI-Anything](agents/cli-anything.md) 相邻）和 [microsoft/intelligent-terminal](https://github.com/microsoft/intelligent-terminal)（1.1k，原生集成 agent 的 Windows Terminal 分支）——但目前都还没达到收录门槛。
- [OpenClaw](agents/openclaw.md) 仍是绝对总数第一，379.2k star（+1.1k）；它已有 profile，但因为这种体量的项目周环比增量噪声太大，不进按增量排名的表。
- 本周仍在涨但没进前 10（按增量）：[Codex CLI](https://github.com/openai/codex) 91.7k（+1.3k）、[Ruflo](agents/ruflo.md) 60.0k（+1.1k）、[openhuman](agents/openhuman.md) 32.5k（+1.0k）、[agentmemory](https://github.com/rohitg00/agentmemory) 23.3k（+0.9k）、[financial-services](https://github.com/anthropics/financial-services) 31.5k（+0.7k）、[CLI-Anything](agents/cli-anything.md) 43.3k（+0.6k）、[scientific-agent-skills](https://github.com/K-Dense-AI/scientific-agent-skills) 28.5k（+0.6k）、[CodeWhale](agents/codewhale.md) 38.6k（+0.6k）、[Kimi Code](agents/kimi-code.md) 2.5k（+0.3k）、[12-factor-agents](https://github.com/humanlayer/12-factor-agents) 23.4k（+0.2k）、[jcode](https://github.com/1jehuang/jcode) 7.1k（+0.1k）、[CoStrict](agents/costrict.md) 4.2k（基本持平）。
