...
| --- | --- | --- | --- | --- | --- |
| #1 | [Hermes Agent](agents/hermes-agent.md) | 129k | +10,800 | 已收录 | 自托管多 agent 环境，仍是绝对增量最大的项目，但相比上周 +58k 已开始放缓 |
| #2（新） | [Codex CLI](agents/codex.md) | 79.5k | +3,900 | 已收录 | OpenAI 终端原生 coding agent——本周飙升的直接动因是 4 月 16 日"Computer Use + 并行多 agent"更新，叠加 GPT-5.5 底层 |
| #3 | [OpenScreen](https://github.com/siddharthvaddem/openscreen) | 34.2k | +1,300 | 不收录 | 仍然热门，但本质上是录屏 / 演示工具，不是 agent |
| #4 | [oh-my-codex](agents/oh-my-codex.md) | 27.2k | +1,100 | 已收录 | Codex CLI 的 orchestration layer，搭着 Codex 浪潮继续爬升 |
| #5 | [oh-my-claudecode](agents/oh-my-claudecode.md) | 32.2k | +800 | 已收录 | 围绕 Claude Code 的工作流和 orchestration layer |
| #6 | [Mercury Agent](agents/mercury-agent.md) | 1.9k | +500 | 已收录 | 进入第二周仍在持续涨——权限硬化的 CLI / Telegram 自托管 agent |
| #7 | [AI Edge Gallery](agents/ai-edge-gallery.md) | 22.4k | +300 | 已收录，但附带范围说明 | 端侧 assistant 沙盒，增长趋于平缓 |
...
- Codex CLI 本周是第二快增长项目——核心事件是 OpenAI 在 4 月 16 日推出的"Codex for (almost) everything"更新：在 GPT-5.5 模型层之上加入 Computer Use（在任意 macOS app 里自动鼠键操作）、并行后台 agent、in-app 浏览器，以及 90+ plugin。
