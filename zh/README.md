...
> **最后更新：** 2026-07-08 · **快照窗口：** 2026-07-02 → 2026-07-08（自上次更新以来的增量，约 6 天，估算） · **Star 总数：** 更新时实时核对

项目名链接指向上游 GitHub 仓库。本仓库已写入的 profile，在"在本仓库中的状态"列单独给出链接。

| 排名 | 项目 | 当前 stars | 快照增量 | 在本仓库中的状态 | 应该怎么读 |
| --- | --- | --- | --- | --- | --- |
| #1&#8288;（=） | [mattpocock/skills](https://github.com/mattpocock/skills) | 160.5k | +6,609 | 候补（Skills 浪潮） | 连续第三个窗口守住增量榜首——Matt Pocock 的 `.claude/skills` 个人技能集越过 160k |
| #2&#8288;（=） | [Superpowers](https://github.com/obra/superpowers) | 249.4k | +5,343 | 已收录 · [profile](agents/superpowers.md) | agentic skills 框架逼近 250k——浪潮的框架锚点持续复利增长 |
| #3&#8288;（↑） | [addyosmani/agent-skills](https://github.com/addyosmani/agent-skills) | 72.8k | +4,296 | 候补（Skills 浪潮） | 本窗口涨幅最大——从 #6 上升三位，Addy Osmani 的 curated agent-skills 集越过 72k |
| #4&#8288;（↓） | [Hermes Agent](https://github.com/NousResearch/hermes-agent) | 211.3k | +3,572 | 已收录 · [profile](agents/hermes-agent.md) | 降温一位到 #4——已收录项目里绝对总数第一，越过 211k |
| #5&#8288;（↓） | [anthropics/skills](https://github.com/anthropics/skills) | 159.4k | +1,779 | 候补（Skills 浪潮源头） | 下滑一位——Anthropic 自家的 `.claude/skills` 参考仓库，上游源头，逼近 160k |
| #6&#8288;（↓） | [colbymchenry/codegraph](https://github.com/colbymchenry/codegraph) | 58.5k | +1,576 | 已收录 · [profile](agents/codegraph.md) | 稳步上行——预索引代码知识图谱越过 58k |
| #7&#8288;（↑） | [Pi](https://github.com/earendil-works/pi) | 68.6k | +1,431 | 已收录 · [profile](agents/pi.md) | 上升一位——Earendil 接手的 harness 越过 68k |
| #8&#8288;（↓） | [TradingAgents](https://github.com/TauricResearch/TradingAgents) | 91.8k | +1,424 | 不收录 | 垂直的金融研究多 agent 系统下滑一位并越过 91k——总数高、增长稳，但作为领域垂直不收录 |
| #9&#8288;（↑） | [Codex CLI](https://github.com/openai/codex) | 96.2k | +1,180 | 已收录 · [profile](agents/codex.md) | 上升一位——OpenAI 的 Codex CLI 逼近 96k |
| #10&#8288;（↓） | [Imbad0202/academic-research-skills](https://github.com/Imbad0202/academic-research-skills) | 36.9k | +948 | 候补（Skills 浪潮） | 下滑一位但仍在涨——面向 Claude Code 的学术研究管线越过 36k |

- 热度适合拿来发现新项目，不适合直接当选型顺序。
- 本窗口头条是 curated skills 的轮换：[addyosmani/agent-skills](https://github.com/addyosmani/agent-skills) 是本窗口涨幅最大的项目（+4.3k），从 #6 上升三位到 #3，把 curated 那一端重新炒热。它上面 [mattpocock/skills](https://github.com/mattpocock/skills) 连续第三个窗口守住 #1 增量位（+6.6k，越过 160k），而 [Hermes Agent](agents/hermes-agent.md) 和 [anthropics/skills](https://github.com/anthropics/skills) 各降温一位。本窗口仍没有黑马——榜首几家就是稳定复利增长。
- `.claude/skills` 浪潮仍占前 10 的 5 个（`mattpocock/skills`、`Superpowers`、`addyosmani/agent-skills`、`anthropics/skills`、`academic-research-skills`）。策略不变：curated 集合作为 Skills 浪潮条目跟踪，框架那一端通过 [Superpowers](agents/superpowers.md) 覆盖。
- [TradingAgents](https://github.com/TauricResearch/TradingAgents) 下滑到 #8（+1.4k，越过 91k），但作为金融研究垂直不收录；[Codex CLI](agents/codex.md) 升到 #9（逼近 96k）、[Pi](agents/pi.md) 升到 #7（越过 68k）。
- **新收录扫描** 本窗口没有新的达到收录门槛的 agent 表面。最新且体量最大的新仓库是 [langchain-ai/openwiki](https://github.com/langchain-ai/openwiki)（9.6k，TypeScript——一个替 agent 写并维护文档的 CLI）和 [vercel/eve](https://github.com/vercel/eve)（3.3k，TypeScript——Vercel 的 agent 构建框架）；两者与延续的 [op7418/guizang-social-card-skill](https://github.com/op7418/guizang-social-card-skill)（4.8k）、[JimLiu/baoyu-design](https://github.com/JimLiu/baoyu-design)（2.5k）、[tigicion/dao-code](https://github.com/tigicion/dao-code)（1.6k）一起留在候补。都还没达到收录门槛。
- [OpenClaw](agents/openclaw.md) 仍是绝对总数第一，382.2k star（+0.7k）；它已有 profile，但因为这种体量的项目周环比增量噪声太大，不进按增量排名的表。
- 本周仍在涨但没进前 10（按增量）：[Ruflo](agents/ruflo.md) 63.5k（+0.9k）、[scientific-agent-skills](https://github.com/K-Dense-AI/scientific-agent-skills) 30.5k（+0.5k）、[CLI-Anything](agents/cli-anything.md) 45.0k（+0.5k）、[openhuman](agents/openhuman.md) 34.4k（+0.4k）、[MiMoCode](agents/mimocode.md) 11.6k（+0.4k）、[agentmemory](https://github.com/rohitg00/agentmemory) 24.8k（+0.3k）、[financial-services](https://github.com/anthropics/financial-services) 33.2k（+0.3k）、[CodeWhale](agents/codewhale.md) 39.6k（+0.3k）、[jcode](https://github.com/1jehuang/jcode) 8.2k（+0.1k）、[Kimi Code](agents/kimi-code.md) 3.0k（+0.1k）、[12-factor-agents](https://github.com/humanlayer/12-factor-agents) 24.0k（+0.1k）、[CoStrict](agents/costrict.md) 4.3k（基本持平）。
