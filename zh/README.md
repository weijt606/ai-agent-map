...
| --- | --- | --- | --- | --- | --- |
| #1（↑） | [mattpocock/skills](https://github.com/mattpocock/skills) | 144.5k | +11,147 | 候补（Skills 浪潮） | 以本窗口最大涨幅夺回增量榜首——Matt Pocock 的 `.claude/skills` 个人技能集越过 144k |
| #2（↑） | [Superpowers](https://github.com/obra/superpowers) | 237.6k | +6,649 | 已收录 · [profile](agents/superpowers.md) | agentic skills 框架突破 237k——浪潮的框架锚点持续复利增长 |
| #3（↑） | [Hermes Agent](https://github.com/NousResearch/hermes-agent) | 201.7k | +5,622 | 已收录 · [profile](agents/hermes-agent.md) | 重新爬回增量表并突破 200k——已收录项目里绝对总数第一 |
| #4（↓） | [addyosmani/agent-skills](https://github.com/addyosmani/agent-skills) | 66.3k | +4,163 | 候补（Skills 浪潮） | 从上周榜首降温但仍在涨——Addy Osmani 的 curated agent-skills 集越过 66k |
| #5（↑） | [colbymchenry/codegraph](https://github.com/colbymchenry/codegraph) | 54.1k | +3,091 | 已收录 · [profile](agents/codegraph.md) | 稳步上行——预索引代码知识图谱越过 54k |
| #6（↑） | [anthropics/skills](https://github.com/anthropics/skills) | 154.7k | +2,531 | 候补（Skills 浪潮源头） | Anthropic 自家的 `.claude/skills` 参考仓库——上游源头；稳定增长越过 154k |
| #7（↑） | [Pi](https://github.com/earendil-works/pi) | 65.3k | +1,737 | 已收录 · [profile](agents/pi.md) | 稳步上行——Earendil 接手的 harness 越过 65k |
| #8 | [Imbad0202/academic-research-skills](https://github.com/Imbad0202/academic-research-skills) | 34.1k | +1,716 | 候补（Skills 浪潮） | 仍是增速很快的 curated skills 集——面向 Claude Code 的学术研究管线越过 34k |
| #9（新） | [Codex CLI](https://github.com/openai/codex) | 93.4k | +1,616 | 已收录 · [profile](agents/codex.md) | 重回增量表——OpenAI 的 Codex CLI 越过 93k |
| #10 | [TradingAgents](https://github.com/TauricResearch/TradingAgents) | 88.3k | +1,396 | 不收录 | 垂直的金融研究多 agent 系统——总数高、增长稳，但作为领域垂直不收录 |

- 热度适合拿来发现新项目，不适合直接当选型顺序。
- 本窗口头条是 [mattpocock/skills](https://github.com/mattpocock/skills)：这个 curated `.claude/skills` 个人集本周拿到所有追踪仓库里最大的涨幅（+11.1k），夺回 #1 增量位并越过 144k。
- 上窗口的黑马 [MiMoCode](agents/mimocode.md) 急速降温——在前一窗口几乎翻三倍后，本周只涨了约 1.0k（9.6k → 10.6k），跌出增量前 10。单窗口黑马很难持续，profile 保留。
- `.claude/skills` 浪潮仍占前 10 的 5 个（`mattpocock/skills`、`Superpowers`、`addyosmani/agent-skills`、`anthropics/skills`、`academic-research-skills`），但榜首在内部轮换：[addyosmani/agent-skills](https://github.com/addyosmani/agent-skills) 随 `mattpocock/skills` 暴涨从 #1 滑到 #4。策略不变：curated 集合作为 Skills 浪潮条目跟踪，框架那一端通过 [Superpowers](agents/superpowers.md) 覆盖。
- [Codex CLI](agents/codex.md) 以 #9 重回增量表（+1.6k，越过 93k）；[Hermes Agent](agents/hermes-agent.md) 爬回 #3 并突破 200k。
- **新收录扫描** 本窗口没有新的达到收录门槛的 agent 表面。最大的几个新仓库都是 curated skill 集合，留在 watchlist——[op7418/guizang-social-card-skill](https://github.com/op7418/guizang-social-card-skill)（4.0k，给 Claude Code / Codex 生成小红书 / 微信卡片的 skill）和 [ruvnet/agent-harness-generator](https://github.com/ruvnet/agent-harness-generator)（0.3k，元 harness 脚手架，与 [OpenHarness](agents/openharness.md) 相邻）——都还没达到收录门槛。
- [OpenClaw](agents/openclaw.md) 仍是绝对总数第一，380.3k star（+1.1k）；它已有 profile，但因为这种体量的项目周环比增量噪声太大，不进按增量排名的表。
- 本周仍在涨但没进前 10（按增量）：[Ruflo](agents/ruflo.md) 61.2k（+1.2k）、[MiMoCode](agents/mimocode.md) 10.6k（+1.0k）、[financial-services](https://github.com/anthropics/financial-services) 32.4k（+0.9k）、[scientific-agent-skills](https://github.com/K-Dense-AI/scientific-agent-skills) 29.2k（+0.7k）、[agentmemory](https://github.com/rohitg00/agentmemory) 23.9k（+0.6k）、[jcode](https://github.com/1jehuang/jcode) 7.7k（+0.6k）、[openhuman](agents/openhuman.md) 32.9k（+0.4k）、[CLI-Anything](agents/cli-anything.md) 43.7k（+0.4k）、[CodeWhale](agents/codewhale.md) 38.9k（+0.3k）、[Kimi Code](agents/kimi-code.md) 2.7k（+0.2k）、[12-factor-agents](https://github.com/humanlayer/12-factor-agents) 23.5k（+0.1k）、[CoStrict](agents/costrict.md) 4.2k（基本持平）。
