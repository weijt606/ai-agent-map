...
| --- | --- | --- | --- | --- | --- |
| #1（↑） | [Hermes Agent](https://github.com/NousResearch/hermes-agent) | 190.5k | +13,900 | 已收录 · [profile](agents/hermes-agent.md) | 连续第七个窗口增长——已收录项目里绝对总数第一，重新夺回增量榜首并突破 190k |
| #2 | [mattpocock/skills](https://github.com/mattpocock/skills) | 125.1k | +10,400 | 候补（Skills 浪潮） | 仍在加速——Matt Pocock 的 `.claude/skills` 个人技能集又涨约 1 万，越过 125k |
| #3（↓） | [colbymchenry/codegraph](https://github.com/colbymchenry/codegraph) | 47.2k | +9,700 | 已收录 · [profile](agents/codegraph.md) | 从上窗口的黑马势头降温，但仍在快速增长——预索引代码知识图谱越过 47k，agent 上下文基础设施持续吸引需求 |
| #4 | [Superpowers](https://github.com/obra/superpowers) | 224.3k | +8,800 | 已收录 · [profile](agents/superpowers.md) | agentic skills 框架突破 224k——浪潮的框架锚点持续复利增长 |
| #5（↑） | [addyosmani/agent-skills](https://github.com/addyosmani/agent-skills) | 53.4k | +5,700 | 候补（Skills 浪潮） | 跃进第一梯队——Addy Osmani 的 curated agent-skills 集加速越过 53k |
| #6（↓） | [Imbad0202/academic-research-skills](https://github.com/Imbad0202/academic-research-skills) | 30.1k | +4,100 | 候补（Skills 浪潮） | 仍是增速很快的 curated skills 集——面向 Claude Code 的学术研究管线越过 30k |
| #7（↓） | [anthropics/skills](https://github.com/anthropics/skills) | 149.3k | +3,800 | 候补（Skills 浪潮源头） | Anthropic 自家的 `.claude/skills` 参考仓库——上游源头；稳定增长，逼近 150k |
| #8（↑） | [TradingAgents](https://github.com/TauricResearch/TradingAgents) | 85.1k | +3,000 | 不收录 | 垂直的金融研究多 agent 系统——总数高、增长稳，但作为领域垂直不收录 |
| #9（↓） | [Pi](https://github.com/earendil-works/pi) | 61.7k | +2,900 | 已收录 · [profile](agents/pi.md) | 稳步上行——Earendil 接手的 harness 越过 61k |
| #10（新） | [Codex CLI](https://github.com/openai/codex) | 90.4k | +2,500 | 不收录（产品面以 [Codex](agents/codex.md) 收录） | OpenAI 的开源编码 CLI 越过 90k——产品面以 [Codex](agents/codex.md) 收录 |

- 热度适合拿来发现新项目，不适合直接当选型顺序。
- 热度表是纯增量刷新；上窗口黑马 [codegraph](agents/codegraph.md) 从 +21k 降温到 +9.7k 但仍在涨，[Hermes Agent](agents/hermes-agent.md) 重新夺回增量榜首。
- **两个新增 in-scope profile——厂商官方 coding CLI：** [Kimi Code](agents/kimi-code.md)（`MoonshotAI/kimi-code`，Moonshot 的 Kimi 原生 CLI，kimi-cli 继任者）和 [MiMoCode](agents/mimocode.md)（`XiaomiMiMo/MiMo-Code`，小米官方 MiMo CLI，内置跨会话记忆，2026-06-10 上线，几天内约 3.1k star）。两者都归入直接执行路线；终端编码 CLI 这个品类现在有了自己的[对比页](comparisons/coding-cli-agents.md)。MiMoCode 是小米第一方 CLI，与只是包装 DeepSeek + MiMo 的社区 [CodeWhale](agents/codewhale.md) TUI 不同。
- `.claude/skills` 浪潮重新扩张：前 10 中有 5 个是 skills 集合或框架（`mattpocock/skills`、`Superpowers`、`addyosmani/agent-skills`、`academic-research-skills`、`anthropics/skills`），比上窗口的 4 个回升，其中 [addyosmani/agent-skills](https://github.com/addyosmani/agent-skills) 跃进前五。策略不变：curated 集合作为 Skills 浪潮条目跟踪，框架那一端通过 [Superpowers](agents/superpowers.md) 覆盖。
- [OpenClaw](agents/openclaw.md) 仍是绝对总数第一，378.1k star（+1.9k）；它已有 profile，但因为这种体量的项目周环比增量噪声太大，不进按增量排名的表。
- 本周仍在涨但没进前 10（按增量）：[agentmemory](https://github.com/rohitg00/agentmemory) 22.3k（+1.7k）、[Ruflo](agents/ruflo.md) 58.9k（+1.5k）、[financial-services](https://github.com/anthropics/financial-services) 30.8k（+1.4k）、[CodeWhale](agents/codewhale.md) 38.0k（+1.4k）、[openhuman](agents/openhuman.md) 31.5k（+1.1k）、[scientific-agent-skills](https://github.com/K-Dense-AI/scientific-agent-skills) 27.9k（+1.0k）、[CLI-Anything](agents/cli-anything.md) 42.7k（+0.9k）、[12-factor-agents](https://github.com/humanlayer/12-factor-agents) 23.2k（+0.3k）、[jcode](https://github.com/1jehuang/jcode) 7.0k（+0.2k）。
