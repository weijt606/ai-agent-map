...
> **最后更新：** 2026-09-09 · **快照窗口：** 2026-09-06 → 2026-09-09（自上次更新以来的增量，**3 天**——周日补刷之后，周三例行节奏从这里恢复，窗口偏短；比较一律按周率讲） · **star 数：** 更新时点抓取

项目名链接指向上游 GitHub 仓库。本目录已收录的 profile，在"目录状态"列单独给出链接。

| 排名 | 项目 | 当前 stars | 快照增量 | 目录状态 | 备注 |
| --- | --- | --- | --- | --- | --- |
| #1&#8288;（=） | [mattpocock/skills](https://github.com/mattpocock/skills) | 257.6k | +3,237 | 候补（Skills 浪潮） | **靠 11 个 star 守住 #1**，自身周率降 48%；越过 **25.7 万** |
| #2&#8288;（新） | [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) | 217.2k | +3,226 | [profile](deepseek-harness.md) | **第一个可测增量就落在 #2**，约 7,530/周，与第一名差不到 1% |
| #3&#8288;（=） | [Superpowers](https://github.com/obra/superpowers) | 283.8k | +1,414 | [profile](superpowers.md) | 周率涨 19%，本窗口唯一加速的 skills 条目 |
| #4&#8288;（↓） | [Hermes Agent](https://github.com/NousResearch/hermes-agent) | 243.7k | +1,232 | [profile](hermes-agent.md) | 破 24 万后的第一个窗口降 32%、退两位 |
| #5&#8288;（新） | [Browser Use](https://github.com/browser-use/browser-use) | 113.9k | +1,228 | [profile](browser-use.md) | **第一个可测增量**直接到 #5；浏览器 agent 路线目前唯一条目 |
| #6&#8288;（↓） | [Pi](https://github.com/earendil-works/pi) | 103.4k | +1,028 | [profile](pi.md) | 再降 10%，越过 **10.3 万** |
| #7&#8288;（新） | [TradingAgents](https://github.com/TauricResearch/TradingAgents) | 103.6k | +924 | 不收录（金融研究垂直） | 周率涨 **189%**，掉榜一期后回来；单个短窗口不读成趋势 |
| #8&#8288;（新） | [OpenCode](https://github.com/anomalyco/opencode) | 206.1k | +916 | [profile](opencode.md) | **第一个可测增量**约 2,140/周，越过 **20.6 万** |
| #9&#8288;（↑） | [Ruflo](https://github.com/ruvnet/ruflo) | 71.8k | +824 | [profile](ruflo.md) | 第二个窗口周率涨 **71%**，确认而非反转 |
| #10&#8288;（↓） | [Codex CLI](https://github.com/openai/codex) | 122.8k | +814 | [profile](codex.md) | 周率涨 7% 却退两位，上面挤进三个新仓库 |
...
- **2026-09-06 → 09-09（3 天窗口）—— 上窗口的新纳入，第一个可测增量就拿走前十里三席** —— 周日补刷之后周三例行节奏从这里恢复，窗口短，所有比较按周率讲。**[DeepSeek Harness](deepseek-harness.md) 进 #2**（约 7,530/周）、**[Browser Use](browser-use.md) 进 #5**、**[OpenCode](opencode.md) 进 #8**，三个都是在它们第一次能进榜的窗口里做到的。这与其读成三个同时爆发，不如读成那次扫描失职终于落到榜面上：三个都体量巨大、活跃了数周，而本榜此前没有它们的行。**#1 是靠 11 个 star 定的** —— [mattpocock/skills](https://github.com/mattpocock/skills) +3,237 对 DeepSeek Harness 的 +3,226 —— 而且第一名自己的周率比上期历史高点降了 48%。**skills 浪潮 6/10 掉到 2/10，是这波浪潮开始以来最低**（2026-05-23 以来地板一直是 3，只在 07-29 出现过一次）；集合整体降温（[anthropics/skills](https://github.com/anthropics/skills) −52%、[academic-research-skills](https://github.com/Imbad0202/academic-research-skills) −37%、[K-Dense](https://github.com/K-Dense-AI/scientific-agent-skills) −27%），唯一例外是 [Superpowers](superpowers.md) 的 +19%。**[Ruflo](ruflo.md) 确认而非反转**，第二个窗口涨 71%，是五个窗口以来第一个扛过下一期的爆发。[TradingAgents](https://github.com/TauricResearch/TradingAgents)（+189%）与 [OpenHands](openhands.md)（+129%）记为尖峰，待第二个窗口确认。里程碑：OpenCode **20.6 万**、DeepSeek Harness **21.7 万**、Pi 与 TradingAgents **10.3 万**、mattpocock **25.7 万**。新收录扫描：**不新增 profile**——没有达到收录门槛的项目，且三天的跨度不足以公平判断一个新项目。候补变动：[AMAP-ML/LongHorizon-Harness](https://github.com/AMAP-ML/LongHorizon-Harness)（1.5k，MIT，长周期计算机操作 harness，原生接 Claude Code / Codex / OpenClaw）是本窗口第一个真正属于 [harness 路线](../comparisons/agent-harness-frameworks.md)的候选；[sepia](https://github.com/Nanako0129/sepia) 从 09-01 的 1.4k 长到 2.5k；[Fuxi](https://github.com/fuxicodex/Fuxi) 到 3.4k，但仍是 `NOASSERTION`，依旧是硬性阻断。榜外还冒出两个 skills 集合，[unlazy](https://github.com/Leonxlnx/unlazy)（3.2k）与 [shuohao-skills](https://github.com/eternityspring/shuohao-skills)（3.2k，短剧制作）——这一点要和本窗口对浪潮的判断放在一起看：浪潮在榜上丢席位、同时又在长出新的领域垂直，两件事是同时发生的。 详见 [market-events](../market-events.md)。
...
- **2026-08-13（补漏收录）—— DeepSeek 开源了一个没有内核的 harness** —— `deepseek-ai/deepseek-harness`（MIT，TypeScript）以 developer preview 形态发布，建在 Cordis 插件内核上：模型适配层、工具注册表、会话日志**以及 agent 循环本身**全都是可从配置替换的插件。它自带 Web UI、headless 运行器、JSON-RPC SDK（TS + Python）与 ACP server，**24 天拿到 21.4 万 star / 2.51 万 fork**。新增 profile：**[DeepSeek Harness](deepseek-harness.md)** —— [harness 路线](../comparisons/agent-harness-frameworks.md)上的第四种形态，与"fork 一个循环""meta-harness""harness 即服务端"并列。按惯例带一个窗口的 `tracked: false`，下次刷新进榜；一个这种体量的仓库在本榜外躺了 24 天，是扫描环节的失职，这里如实记一笔。详见 [market-events](../market-events.md)。
