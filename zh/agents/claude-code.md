...
- **2026-07-07 起**：Fable 5 不再包含在订阅内，改为按量计费的 usage credits——Claude Code 的实际能力上限现在取决于你的额度预算。
- **2026-07-24 —— [Claude Opus 5](claude-opus-5.md)**：接替 Opus 4.8 成为按量天花板之下的可靠默认档——价格仍是 $5/$25，1M 上下文既是默认也是上限，并成为 Claude Max 的默认模型。被安全分类器拦下的请求仍然回退到 Opus 4.8。
- **2026-09-01 —— [Fable 5.1](claude-fable-5.md)**：缓存读砍 75% 到 $0.25/M——那正是一个长 Claude Code 会话账单真正堆积的地方——外加网安类误报少约 60%。
- **2026-08-06 —— 自托管环境（公开 beta）**：会话可以跑在你自己的基础设施上，在你自己的网络里、紧挨内部服务与工具链，而不是 Anthropic 托管的算力。仅限 Team 与 Enterprise 计划，**默认关闭**，使用零数据保留（ZDR）的组织不可用。这缩小了"什么时候不要选它"里点出的那道口子，但没有填平：执行可以是你的了，模型和客户端仍然不是。

## 使用代价

复杂度是 Medium。上手不算难，真正的门槛在于怎么写好项目指令、怎么限制权限、怎么把 MCP 和 subagents 用得不混乱。2026 年 7 月起，"花 [Fable 5.1](claude-fable-5.md) 额度 vs 留在 [Opus 5](claude-opus-5.md)" 也是使用成本决策的一部分。

## 最后一句

Claude Code 更像“长期可用的开发增强层”，不是一次性任务投递箱——而且 2026 年年中之后，它的能力上限变成了按任务做的模型选择（花额度用 [Fable 5.1](claude-fable-5.md)，默认用 [Opus 5](claude-opus-5.md)），不再是固定属性。