...
[![EN](https://img.shields.io/badge/EN-English-2563eb?style=for-the-badge&labelColor=1d4ed8)](../../capabilities/README.md)
...
这些维度被逐一应用到每个项目、并排放在**[能力矩阵](matrix.md)**里——按每项能力是核心强项（●）、扎实支持（◐）、有限（○）还是非目标（—）打分。当你在多个项目之间做选择、而不是读单个 profile 时，从那里开始。

光一个符号无法核实、也无法反驳，所以格子可以带一条**[证据记录](evidence-records.md)**——符号，加上在哪、什么时候、对哪个版本核对的，以及证据是*宣传的*还是*实测的*。schema、记忆生命周期子 schema 和分阶段推进都在那一页。